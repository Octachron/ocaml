      ...
      (fun () ->
         ...
         let (+-+) flam (name, pass) =
           ...
           if Clflags.Dump_option.get Flambda_verbose then begin
             let log fmt =
               Log.itemf Dump_log.flambda log fmt in
             log  "@.PASS: %s@." name;
             ...
             Profile.record_call ~accumulate:true "closure_conversion"
               (fun () ->
                  ...
                  |> Closure_conversion.lambda_to_flambda ~backend
                       ~module_ident ~size)
           in
           Clflags.dump_item_on_log log Dump_log.raw_flambda
               ...
               Location.prerr_warning (Debuginfo.to_location apply.dbg)
                 (Warnings.Inlining_impossible
                    "[@unrolled] attribute was not used on this function \
                     application (the optimizer did not know what function \
                     was being applied)"));
           Clflags.dump_item_on_log log Dump_log.flambda
             "End of middle end:@ %a@."
             Flambda.print_program flam;
           check flam;
...
let flambda_raw_clambda_dump_if log
  ...
  if Clflags.Dump_option.get Raw_clambda then
    begin
      let log fmt = Log.itemf Dump_log.raw_clambda log fmt in
  ...
  Clflags.dump_item_on_log log Dump_log.cmm "@.cmm:@.";
  input
