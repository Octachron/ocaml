...
let style = function
  | Preservation -> Misc.Style.Preservation
  | Deletion -> Misc.Style.Deletion
  | Insertion -> Misc.Style.Insertion
  | Modification -> Misc.Style.Modification

let prefix ppf (pos, p) =
  let open Format_doc in
  pp_open_stag ppf (style p);
  fprintf ppf "%i. " pos;
  pp_close_stag ppf ()
