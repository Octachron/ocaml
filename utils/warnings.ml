...
let space_inline_list = Fmt.(pp_print_list Style.inline_code)
...
let message = function
  ...
  | Method_override (cname :: slist) ->
      msg "the following methods are overridden by the class %a:@ %a"
  ...
  | Missing_record_field_pattern s ->
      msg "the following labels are not bound in this record pattern:@ %a .@ \
  ...
  | Instance_variable_override (cname :: slist) ->
      msg
      "the following instance variables are overridden by the class %a:@ %a"
      Style.inline_code cname
      space_inline_list slist
  ...
  | Implicit_public_methods l ->
      msg "the following private methods were made public implicitly:@ %a."
      space_inline_list l
  ...
  | Ambiguous_name ([s], tl, false, expansion) ->
      msg "%a belongs to several types: %a.@ \
  ...
  | Ambiguous_name (_slist, tl, true, expansion) ->
      msg "these field labels belong to several types:@ %a.@ \
  ...
  | Ambiguous_var_in_pattern_guard vars ->
      ...
      let vars_explanation =
        ...
        | [] -> assert false
        | [x] ->
            Fmt.dprintf "variable %a appears %s"
              Style.inline_code x
              in_different_places
        | _::_ ->
            Fmt.dprintf "variables %a appear %s"
