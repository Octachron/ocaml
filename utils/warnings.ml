...
let quoted_list ~sep ppf ls =
  Format_doc.pp_print_list ~pp_sep:sep Format_doc.pp_print_string ppf ls

let inline_code = Format_doc.pp_print_string
let as_inline_code pr = pr
...
let message = function
  ...
  | Duplicate_definitions (kind, cname, tc1, tc2) ->
      Format_doc.doc_printf
        ...
        kind inline_code cname
        inline_code tc1
        inline_code tc2
  | Unused_value_declaration v ->
      Format_doc.doc_printf "unused value %a." inline_code v
  | Unused_open s ->
      Format_doc.doc_printf "unused open %a." inline_code s
  | Unused_open_bang s ->
      Format_doc.doc_printf "unused open! %a." inline_code s
  | Unused_type_declaration s ->
      Format_doc.doc_printf "unused type %a." inline_code s
  | Unused_for_index s ->
      Format_doc.doc_printf  "unused for-loop index %a."
        inline_code s
  | Unused_ancestor s ->
      Format_doc.doc_printf "unused ancestor variable %a." inline_code s
  | Unused_constructor (s, Unused) ->
      Format_doc.doc_printf "unused constructor %a." inline_code s
  ...
  | Unused_extension (s, is_exception, complaint) ->
      ...
      | Unused ->
          Format_doc.doc_printf "unused %s %a" kind inline_code s
      | Not_constructed ->
          Format_doc.doc_printf
