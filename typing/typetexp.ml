...
let report_error env ppf =
  ...
  | Unbound_type_variable name ->
     (* we don't use "spellcheck" here: the function that raises this
        ...
        should be handled *)
    I18n.fprintf ppf "Unbound type parameter %s@." name
  | Unbound_type_constructor lid ->
    I18n.fprintf ppf "Unbound type constructor %a" longident lid;
  ...
  | Bound_type_variable name ->
    I18n.fprintf ppf "Already bound type parameter '%s" name
  | Recursive_type ->
  ...
  | Present_has_no_type l ->
      I18n.fprintf ppf "The present constructor %s has no type" l
  ...
  | Invalid_variable_name name ->
      I18n.fprintf ppf "The type variable name %s is not allowed in programs" name
  | Cannot_quantify (name, v) ->
      I18n.fprintf ppf
        "@[<hov>The universal type variable '%s cannot be generalized:@ %a.@]"
        name I18n.pp
        (if Btype.is_Tvar v then I18n.s"it escapes its scope" else
         if Btype.is_Tunivar v then I18n.s"it is already bound to another variable"
         else I18n.s"it is not a variable")
  ...
  | Unbound_value lid ->
      I18n.fprintf ppf "Unbound value %a" longident lid;
      spellcheck ppf fold_values env lid;
  | Unbound_module lid ->
      I18n.fprintf ppf "Unbound module %a" longident lid;
      spellcheck ppf fold_modules env lid;
  | Unbound_constructor lid ->
      I18n.fprintf ppf "Unbound constructor %a" longident lid;
      spellcheck ppf fold_constructors env lid;
  | Unbound_label lid ->
      I18n.fprintf ppf "Unbound record field %a" longident lid;
      spellcheck ppf fold_labels env lid;
  | Unbound_class lid ->
      I18n.fprintf ppf "Unbound class %a" longident lid;
      spellcheck ppf fold_classs env lid;
  | Unbound_modtype lid ->
      I18n.fprintf ppf "Unbound module type %a" longident lid;
      spellcheck ppf fold_modtypes env lid;
  | Unbound_cltype lid ->
      I18n.fprintf ppf "Unbound class type %a" longident lid;
      spellcheck ppf fold_cltypes env lid;
  | Ill_typed_functor_application lid ->
      I18n.fprintf ppf "Ill-typed functor application %a" longident lid
  | Illegal_reference_to_recursive_module ->
      I18n.fprintf ppf "Illegal recursive module reference"
  | Access_functor_as_structure lid ->
      I18n.fprintf ppf "The module %a is a functor, not a structure" longident lid
  | Apply_structure_as_functor lid ->
      I18n.fprintf ppf "The module %a is a structure, not a functor" longident lid
