...
let expression sub exp =
  ...
  let desc =
    ...
    | Texp_function { arg_label; cases = [{c_lhs=p; c_guard=None; c_rhs=e}];
         _ } ->
        let arg_label = match arg_label with
          | Nolabel | Labelled _ as a -> a
          | Optional (s,()) -> Optional(s,None)
          | Typed_optional (_s,()) -> assert false (* DELENDA *) in
        Pexp_fun (arg_label, sub.pat sub p, sub.expr sub e)
    ...
    | Texp_function ({arg_label=Optional (s,_)| Labelled s | Typed_optional (s,_)
                    ;cases; _ } as t) ->
        let label = match t.arg_label with
          | Nolabel | Labelled _ as a -> a
          | Optional (s,()) -> Optional(s,None)
          | Typed_optional (_s,()) -> assert false (* DELENDA *) in
        let name = fresh_name s exp.exp_env in
        Pexp_fun (label, Pat.var ~loc {loc;txt = name },
  ...
  let desc = match cexpr.cl_desc with
    ...
    | Tcl_fun (label, pat, _pv, cl, _partial) ->
        let label =  match label with
          ...
          | Optional (s,()) -> Optional(s,None)
          | Typed_optional (_s,()) -> assert false (* DELENDA *) in
        Pcl_fun (label, sub.pat sub pat, sub.class_expr sub cl)
