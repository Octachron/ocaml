  ...
  if raw_status.unboxed && not raw_status.default then begin
    ...
    | Ptype_abstract ->
        raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                      (I18n.s "it is abstract")))
    | Ptype_variant [{pcd_args = Pcstr_tuple []; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "its constructor has no argument")))
    | Ptype_variant [{pcd_args = Pcstr_tuple [_]; _}] -> ()
    | Ptype_variant [{pcd_args = Pcstr_tuple _; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "its constructor has more than one argument")))
    | Ptype_variant [{pcd_args = Pcstr_record
                        [{pld_mutable=Immutable; _}]; _}] -> ()
    | Ptype_variant [{pcd_args = Pcstr_record [{pld_mutable=Mutable; _}]; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute (I18n.s "it is mutable")))
    | Ptype_variant [{pcd_args = Pcstr_record _; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "its constructor has more than one argument")))
    | Ptype_variant _ ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "it has more than one constructor")))
    | Ptype_record [{pld_mutable=Immutable; _}] -> ()
    | Ptype_record [{pld_mutable=Mutable; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "it is mutable")))
    | Ptype_record _ ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "it has more than one field")))
    | Ptype_open ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (I18n.s "extensible variant types cannot be unboxed")))
  end;
  let unboxed_status =
    match sdecl.ptype_kind with
    | Ptype_variant [{pcd_args = Pcstr_tuple [_]; _}]
...
let report_error ppf =
  ...
  | Rebind_wrong_type (lid, env, trace) ->
      Printtyp.report_unification_error ppf env trace
        (fun ppf -> I18n.fprintf ppf "The constructor %a@ has type"
         Printtyp.longident lid)
        (fun ppf -> I18n.fprintf ppf "but was expected to be of type")
  ...
  | Bad_variance (n, v1, v2) ->
      ...
      in
      let expected ppf =
        I18n.fprintf ppf "It was expected to be %a,@ but it is %a."
          I18n.pp (variance v2)  I18n.pp (variance v1) in
      if n = -1 then
        I18n.fprintf ppf
  ...
  | Unavailable_type_constructor p ->
      I18n.fprintf ppf "The definition of type %a@ is unavailable" Printtyp.path p
  | Bad_fixed_type r ->
  ...
  | Cannot_unbox_or_untag_type Unboxed ->
      I18n.fprintf ppf "Don't know how to unbox this type. Only float, int32, \
                   int64 and nativeint can be unboxed"
  | Cannot_unbox_or_untag_type Untagged ->
      I18n.fprintf ppf "Don't know how to untag this type. Only int \
                      can be untagged"
  | Deep_unbox_or_untag_attribute kind ->
      I18n.fprintf ppf
  ...
  | Boxed_and_unboxed ->
      I18n.fprintf ppf "@[A type cannot be boxed and unboxed at the same time.@]"
  | Nonrec_gadt ->
      I18n.fprintf ppf
