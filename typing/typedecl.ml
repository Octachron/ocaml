...
let transl_declaration env sdecl id =
  ...
  if raw_status.unboxed && not raw_status.default then begin
    ...
    | Ptype_variant [{pcd_args = Pcstr_tuple []; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"its constructor has no argument")))
    | Ptype_variant [{pcd_args = Pcstr_tuple [_]; _}] -> ()
    | Ptype_variant [{pcd_args = Pcstr_tuple _; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"its constructor has more than one argument")))
    | Ptype_variant [{pcd_args = Pcstr_record
                        [{pld_mutable=Immutable; _}]; _}] -> ()
    | Ptype_variant [{pcd_args = Pcstr_record [{pld_mutable=Mutable; _}]; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute (s_"it is mutable")))
    | Ptype_variant [{pcd_args = Pcstr_record _; _}] ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"its constructor has more than one argument")))
    | Ptype_variant _ ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"it has more than one constructor")))
    ...
    | Ptype_record _ ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"it has more than one field")))
    | Ptype_open ->
      raise(Error(sdecl.ptype_loc, Bad_unboxed_attribute
                    (s_"extensible variant types cannot be unboxed")))
  end;
  let unboxed_status =
    match sdecl.ptype_kind with
    | Ptype_variant [{pcd_args = Pcstr_tuple [_]; _}]
...
let explain_unbound_gen ppf tv tl typ kwd pr =
  try
    ...
    fprintf ppf
      (f_".@.@[<hov2>In %a@ %a@;<1 -2>the variable %a is unbound@]")
...
let explain_unbound_single ppf tv ty =
  let trivial ty =
    explain_unbound ppf tv [ty] (fun t -> t) (s_"type") (fun _ -> "") in
  match (Ctype.repr ty).desc with
    Tobject(fi,_) ->
      let (tl, rv) = Ctype.flatten_fields fi in
      if rv == tv then trivial ty else
      explain_unbound ppf tv tl (fun (_,_,t) -> t)
        (s_"method") (fun (lab,_,_) -> lab ^ ": ")
  | Tvariant row ->
      let row = Btype.row_repr row in
...
let report_error ppf =
  let open I18n in let open I18n_core in
  function
  | Repeated_parameter ->
      fprintf ppf (f_"A type parameter occurs several times")
  | Duplicate_constructor s ->
      fprintf ppf (f_"Two constructors are named %s") s
  | Too_many_constructors ->
      fprintf ppf
        (f_"@[Too many non-constant constructors@ -- maximum is %i %a@]")
        (Config.max_tag + 1) i18n (s_"non-constant constructors")
  | Duplicate_label s ->
      fprintf ppf (f_"Two labels are named %s") s
  | Recursive_abbrev s ->
      fprintf ppf (f_"The type abbreviation %s is cyclic") s
  | Cycle_in_def (s, ty) ->
      Printtyp.reset_and_mark_loops ty;
      fprintf ppf (f_"@[<v>The definition of %s contains a cycle:@ %a@]")
        s Printtyp.type_expr ty
  | Definition_mismatch (ty, errs) ->
      Printtyp.reset_and_mark_loops ty;
      Format.fprintf ppf "@[<v>@[<hov>%a@ %a@;<1 2>%a@]%a@]"
        i18n (s_"This variant or record definition")
        i18n (s_"does not match that of type")
        Printtyp.type_expr ty
        (Includecore.report_type_mismatch (s_"the original") (s_"this")
           (s_"definition"))
        errs
  | Constraint_failed (ty, ty') ->
      Printtyp.reset_and_mark_loops ty;
      Printtyp.mark_loops ty';
      fprintf ppf (f_"@[%a@ @[<hv>Type@ %a@ should be an instance of@ %a@]@]")
        i18n (s_"Constraints are not satisfied in this type.")
        Printtyp.type_expr ty Printtyp.type_expr ty'
  | Parameters_differ (path, ty, ty') ->
      Printtyp.reset_and_mark_loops ty;
      Printtyp.mark_loops ty';
      fprintf ppf
        (f_"@[<hv>In the definition of %s, type@ %a@ should be@ %a@]")
        (Path.name path) Printtyp.type_expr ty Printtyp.type_expr ty'
  | Inconsistent_constraint (env, trace) ->
      fprintf ppf (f_"The type constraints are not consistent.@.");
      Printtyp.report_unification_error ppf env trace
        (dprintf @@ f_"Type")
        (dprintf @@ f_"is not compatible with type")
  | Type_clash (env, trace) ->
      Printtyp.report_unification_error ppf env trace
        (dprintf @@ f_"This type constructor expands to type")
        (dprintf @@ f_"but is used here with type")
  | Null_arity_external ->
      fprintf ppf (f_"External identifiers must be functions")
  | Missing_native_external ->
      fprintf ppf (f_"@[<hv>An external function with more than 5 arguments \
                   requires a second stub function@ \
                   for native-code compilation@]")
  | Unbound_type_var (ty, decl) ->
      fprintf ppf (f_"A type variable is unbound in this type declaration");
      let ty = Ctype.repr ty in
      ...
      | Type_record (tl, _), _ ->
          explain_unbound ppf ty tl (fun l -> l.Types.ld_type)
            (s_"field") (fun l -> Ident.name l.Types.ld_id ^ ": ")
      | Type_abstract, Some ty' ->
          explain_unbound_single ppf ty ty'
      | _ -> ()
      end
  | Unbound_type_var_ext (ty, ext) ->
      fprintf ppf (f_"A type variable is unbound in this extension constructor");
      let args = tys_of_constr_args ext.ext_args in
      explain_unbound ppf ty args (fun c -> c) (s_"type") (fun _ -> "")
  | Not_open_type path ->
      Format.fprintf ppf "@[%a@ %a@]"
        i18n (s_"Cannot extend type definition")
        Printtyp.path path
  | Not_extensible_type path ->
      Format.fprintf ppf "@[%a@ %a@ %a@]"
        i18n (s_"Type")
        Printtyp.path path
        i18n (s_"is not extensible")
  | Extension_mismatch (path, errs) ->
      Format.fprintf ppf "@[<v>@[<hov>%a@ %a@;<1 2>%s@]%a@]"
        i18n (s_"This extension")
        i18n (s_"does not match the definition of type")
        (Path.name path)
        (Includecore.report_type_mismatch
           (s_"the type") (s_"this extension") (s_"definition"))
        errs
  | Rebind_wrong_type (lid, env, trace) ->
      Printtyp.report_unification_error ppf env trace
        (dprintf (f_"The constructor %a@ has type") <$>
         Printtyp.longident <$> lid)
        (dprintf @@ f_"but was expected to be of type")
  | Rebind_mismatch (lid, p, p') ->
      fprintf ppf
        (f_"@[The constructor %a extends type %s whose declarations dos not \
            match the declaration of type %s@]")
        Printtyp.longident lid
        (Path.name p)
        (Path.name p')
  | Rebind_private lid ->
      fprintf ppf (f_"@[The constructor@ %a@ is private@]")
        Printtyp.longident lid
  | Bad_variance (n, v1, v2) ->
      ...
      if n = -1 then
        ...
        fprintf ppf
          ...
          n
          i18n (variance v2) i18n (variance v1)
  | Unavailable_type_constructor p ->
      fprintf ppf (f_"The definition of type %a@ is unavailable") Printtyp.path p
  | Bad_fixed_type r ->
      fprintf ppf (f_"This fixed type %s") r
  | Varying_anonymous ->
      fprintf ppf
        (f_"@[In this GADT definition,@ the variance of some parameter@ \
         cannot be checked@]")
  | Val_in_structure ->
      fprintf ppf (f_"Value declarations are only allowed in signatures")
  | Multiple_native_repr_attributes ->
      fprintf ppf (f_"Too many [@@unboxed]/[@@untagged] attributes")
  | Cannot_unbox_or_untag_type Unboxed ->
      fprintf ppf (f_"Don't know how to unbox this type. Only float, int32, \
                   int64 and nativeint can be unboxed")
  | Cannot_unbox_or_untag_type Untagged ->
      fprintf ppf (f_"Don't know how to untag this type. Only int \
                      can be untagged")
  | Deep_unbox_or_untag_attribute kind ->
      fprintf ppf
        (f_"The attribute '%s' should be attached to a direct argument or \
         result of the primitive, it should not occur deeply into its type")
        (match kind with Unboxed -> "@unboxed" | Untagged -> "@untagged")
  | Bad_immediate_attribute ->
      fprintf ppf
        (f_"@[Types marked with the immediate attribute must be@ \
         non-pointer types like int or bool@]")
  | Bad_unboxed_attribute msg ->
      fprintf ppf (f_"@[This type cannot be unboxed because@ %a.@]") I18n.pp msg
  | Wrong_unboxed_type_float ->
      fprintf ppf (f_"@[This type cannot be unboxed because@ \
                      it might contain both float and non-float values.@ \
                      You should annotate it with [%@%@ocaml.boxed].@]")
  | Boxed_and_unboxed ->
      fprintf ppf (f_"@[A type cannot be boxed and unboxed at the same time.@]")
  | Nonrec_gadt ->
      fprintf ppf
       (f_ "@[GADT case syntax cannot be used in a 'nonrec' block.@]")
