...
let expand_any_trace map env trace =
  let expand_desc x = match x.Errortrace.expanded with
    | None ->
      ...
      Errortrace.{ t = x.t; expanded = Some expanded }
    | Some _ -> x in
...
and unify_row env row1 row2 =
  ...
  begin try
    ...
    List.iter
      (fun (l,f1,f2) ->
        try unify_row_field env fixed1 fixed2
            rm1 rm2 l f1 f2
        with Unify trace ->
...
let filter_method env name priv ty =
  ...
  match get_desc ty with
    Tvar _ ->
      ...
      update_level_for Unify env (get_level ty) ty';
      update_scope_for Unify (get_scope ty) ty';
      link_type ty ty';
...
and moregen_row inst_nongen type_pairs env row1 row2 =
  ...
  begin match get_desc rm1, get_desc rm2 with
    Tunivar _, Tunivar _ ->
