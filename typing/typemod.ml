          ...
          {desc = Tpackage (p, nl, tl)} ->
            ...
            then
              Location.prerr_warning smod.pmod_loc
                (Warnings.Not_principal (s_"this module unpacking"));
...
let report_error ppf = function
    Cannot_apply mty ->
      fprintf ppf
        (f_"@[This module is not a functor; it has type@ %a@]") modtype mty
  | Not_included errs ->
      fprintf ppf
        (f_"@[<v>Signature mismatch:@ %a@]") Includemod.report_error errs
  | Cannot_eliminate_dependency mty ->
      fprintf ppf
        (f_"@[This functor has type@ %a@ \
           The parameter cannot be eliminated in the result type.@  \
           Please bind the argument to a module identifier.@]") modtype mty
  | Signature_expected -> fprintf ppf (f_"This module type is not a signature")
  | Structure_expected mty ->
      fprintf ppf
        (f_"@[This module is not a structure; it has type@ %a") modtype mty
  | With_no_component lid ->
      fprintf ppf
        (f_"@[The signature constrained by `with' has no component named %a@]")
        longident lid
  ...
  | Repeated_name(kind, name) ->
      fprintf ppf
        (f_"@[Multiple definition of the %s name %s.@ \
           Names must be unique in a given structure or signature.@]") kind name
  | Non_generalizable typ ->
      fprintf ppf
        (f_"@[The type of this expression,@ %a,@ \
  ...
  | Non_generalizable_module mty ->
      fprintf ppf
        (f_"@[The type of this module,@ %a,@ \
           contains type variables that cannot be generalized@]") modtype mty
  | Implementation_is_required intf_name ->
      fprintf ppf
        (f_"@[The interface %a@ declares values, not just types.@ \
           An implementation must be provided.@]")
        Location.print_filename intf_name
  | Interface_not_compiled intf_name ->
      fprintf ppf
        (f_"@[Could not find the .cmi file for interface@ %a.@]")
        Location.print_filename intf_name
  | Not_allowed_in_functor_body ->
      fprintf ppf
        (f_"@[This expression creates fresh types.@ \
            It is not allowed inside applicative functors.@]")
  | With_need_typeconstr ->
      fprintf ppf
       (f_"Only type constructors with identical parameters can be substituted.")
  | Not_a_packed_module ty ->
      fprintf ppf
        (f_"This expression is not a packed module. It has type@ %a")
        type_expr ty
  | Incomplete_packed_module ty ->
      fprintf ppf
        (f_"The type of this packed module contains variables:@ %a")
        type_expr ty
  | Scoping_pack (lid, ty) ->
      fprintf ppf
        (f_"The type %a in this module cannot be exported.@ ") longident lid;
      fprintf ppf
        (f_"Its type contains local dependencies:@ %a") type_expr ty
  | Recursive_module_require_explicit_type ->
      fprintf ppf (f_"Recursive modules require an explicit module type.")
  | Apply_generative ->
      fprintf ppf (f_"This is a generative functor. It can only be applied to ()")
  | Cannot_scrape_alias p ->
      fprintf ppf
        (f_"This is an alias for module %a, which is missing")
        path p
