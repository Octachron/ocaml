...
let _highlight = Oprint.highlight
...
let rec tree_of_typexp next mode ty =
  ...
  let pr_typ () =
    ...
    | Tvariant row ->
        ...
        let tags () =
          if all_present then None
          else Some (List.map (fun (x,_) -> plain x) present)
        in
  ...
  | Cty_constr (p', tyl, cty) ->
      ...
      then
        ...
        Octy_constr (
          ...
          tree_of_typlist [] Type_scheme tyl
        )
  | Cty_signature sign ->
      let px = proxy sign.csig_self_row in
      let self_ty =
...
let tree_of_typexp mode typ = tree_of_typexp [] mode typ
