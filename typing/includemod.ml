...
let show_loc msg ppf loc =
  ...
  else Format.fprintf ppf "@\n@[<2>%a:@ %a@]" Location.print_loc loc i18n msg

let show_locs ppf (loc1, loc2) =
  show_loc (s_"Expected declaration") ppf loc2;
  show_loc (s_"Actual declaration") ppf loc1

let include_err ppf = function
  | Missing_field (id, loc, kind) ->
      fprintf ppf (f_"The %s `%a' is required but not provided") kind ident id;
      show_loc (s_"Expected declaration") ppf loc
  | Value_descriptions(id, d1, d2) ->
      fprintf ppf
        (f_"@[<hv 2>Values do not match:@ %a@;<1 -2>is not included in@ %a@]")
        (value_description id) d1 (value_description id) d2;
      show_locs ppf (d1.val_loc, d2.val_loc);
  | Type_declarations(id, d1, d2, errs) ->
      Format.fprintf ppf "@[<v>@[<hv>%a:@;<1 2>%a@ %a@;<1 2>%a@]%a%a@]"
        i18n (s_"Type declarations do not match")
        (type_declaration id) d1
        i18n (s_"is not included in")
        (type_declaration id) d2
        show_locs (d1.type_loc, d2.type_loc)
        (Includecore.report_type_mismatch
           (s_"the first") (s_"the second") (s_"declaration")) errs
  | Extension_constructors(id, x1, x2) ->
      fprintf ppf
       (f_"@[<hv 2>Extension declarations do not match:@ \
  ...
  | Module_types(mty1, mty2)->
      fprintf ppf
       (f_"@[<hv 2>Modules do not match:@ \
        %a@;<1 -2>is not included in@ %a@]")
      modtype mty1
      modtype mty2
  | Modtype_infos(id, d1, d2) ->
      fprintf ppf
       (f_"@[<hv 2>Module type declarations do not match:@ \
           %a@;<1 -2>does not match@ %a@]")
      (modtype_declaration id) d1
      (modtype_declaration id) d2
  | Modtype_permutation ->
      fprintf ppf (f_"Illegal permutation of structure fields")
  | Interface_mismatch(impl_name, intf_name) ->
      fprintf ppf
        (f_"@[The implementation %s@ does not match the interface %s:")
       impl_name intf_name
  | Class_type_declarations(id, d1, d2, reason) ->
      fprintf ppf
       (f_"@[<hv 2>Class type declarations do not match:@ \
  ...
  | Class_declarations(id, d1, d2, reason) ->
      fprintf ppf
       (f_"@[<hv 2>Class declarations do not match:@ \
           %a@;<1 -2>does not match@ %a@]@ %a")
      (Printtyp.class_declaration id) d1
      (Printtyp.class_declaration id) d2
      Includeclass.report_error reason
  | Unbound_modtype_path path ->
      fprintf ppf (f_"Unbound module type %a") Printtyp.path path
  | Unbound_module_path path ->
      fprintf ppf (f_"Unbound module %a") Printtyp.path path
  | Invalid_module_alias path ->
      fprintf ppf (f_"Module %a cannot be aliased") Printtyp.path path

let rec context ppf = function
    Module id :: rem ->
      Format.fprintf ppf "@[<2>module %a%a@]" ident id args rem
  | Modtype id :: rem ->
      Format.fprintf ppf "@[<2>module type %a =@ %a@]" ident id context_mty rem
  | Body x :: rem ->
      Format.fprintf ppf "functor (%s) ->@ %a" (argname x) context_mty rem
  | Arg x :: rem ->
      Format.fprintf ppf "functor (%a : %a) -> ..." ident x context_mty rem
  | [] ->
      fprintf ppf @@ f_"<here>"
...
and args ppf = function
  ...
  | Arg x :: rem ->
      Format.fprintf ppf "(%a :@ %a) : ..." ident x context_mty rem
  | cxt ->
...
let context ppf cxt =
  ...
  if List.for_all (function Module _ -> true | _ -> false) cxt then
    fprintf ppf (f_"In module %a:@ ") path (path_of_context cxt)
  else
    fprintf ppf (f_"@[<hv 2>At position@ %a@]@ ") context cxt

let include_err ppf (cxt, env, err) =
  Printtyp.wrap_printing_env env (fun () ->
    Format.fprintf ppf "@[<v>%a%a@]" context (List.rev cxt) include_err err)
...
let report_error ppf errs =
  ...
  let include_err' ppf (_,_,obj as err) =
    ...
    else if !pe then (fprintf ppf (f_"...@ "); pe := false)
  in
  let print_errs ppf = List.iter (include_err' ppf) in
  Format.fprintf ppf "@[<v>%a%a@]" print_errs errs include_err err
