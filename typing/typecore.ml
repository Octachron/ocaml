  ...
  | Ppat_construct(lid, sarg) ->
      let expected_type =
        try
          ...
          let principal =
            get_level expected_ty = generic_level || not !Clflags.principal
          in
            Some (p0, p, principal)
        with Not_found -> None
      in
      ...
      let ty_record, expected_type =
        let get_path ty =
          try
            ...
            let principal =
              get_level ty = generic_level || not !Clflags.principal
            in
            Some (p0, p, principal)
          with Not_found -> None
        in
...
and type_label_access env srecord usage lid =
  ...
  let expected_type =
    try
      ...
      Some(p0, p, get_level ty_exp = generic_level || not !Clflags.principal)
    with Not_found -> None
  in
...
and type_construct env loc lid sarg ty_expected_explained attrs =
  ...
  let expected_type =
    try
      ...
      let principal =
        get_level ty_expected = generic_level || not !Clflags.principal
      in
      Some(p0, p, principal)
    with Not_found -> None
  in
...
let report_literal_type_constraint const = function
  | Some tr ->
      begin match get_desc Errortrace.(tr.expected.t) with
        Tconstr (typ, [], _) ->
