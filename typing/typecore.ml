...
let report_this_function ppf funct =
  match Typedtree.nominal_exp_doc funct with
  | None -> Format.fprintf ppf "This function"
  | Some name ->
    Format.fprintf ppf "The function '%a'" Format.pp_doc name
...
let report_error ~loc env =
  ...
  | Label_mismatch(lid, err) ->
      report_unification_error ~loc env err
        (Format.doc_printf
           "The record field %a@ belongs to the type" longident lid)
        (Format.doc_printf  "but is mixed here with fields of type")
  | Pattern_type_clash (err, pat) ->
      ...
      report_unification_error ~loc ~sub env err
        (Format.doc_printf "This pattern matches values of type")
        (Format.doc_printf "but a pattern was expected which matches values of \
                            type");
  | Or_pattern_type_clash (id, err) ->
      report_unification_error ~loc env err
        ...
        (Format.doc_printf "but on the right-hand side it has type")
  ...
  | Expr_type_clash (err, explanation, exp) ->
      ...
      report_unification_error ~loc ~sub env err
        ...
        (Format.doc_printf "This expression has type")
        (Format.doc_printf "but an expression was expected of type");
  ...
  | Letop_type_clash(name, err) ->
      report_unification_error ~loc env err
        (Format.doc_printf  "The operator %s has type" name)
        (Format.doc_printf  "but it was expected to have type")
