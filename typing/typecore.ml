...
let check_univars env kind exp ty_expected vars =
    ...
    with_local_level_generalize begin fun () ->
      match get_desc pty with
        Tpoly (body, tl) ->
          ...
          let _, ty' = instance_poly ~keep_names:true ~fixed:true tl body in
  ...
  in
  let ty, status = polyfy env exp_ty vars in
  match status with
  | Ok () -> ()
  | Error missing ->
    ...
    let trace =
      Errortrace.(diff d (root_explanation (Univar_quantification_mismatch missing))) in
