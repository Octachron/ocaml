...
let string_of_path p =
  Final_format.asprintf "%a" (Format.format_printer path) p
...
let raw_type_expr = Raw.raw_type_expr

let () = Btype.print_raw := Raw.raw_type_expr
...
let type_expansion ppf = function
  | Same t -> Oprint.(print out_type) ppf t
  | Diff(t,t') ->
      fprintf ppf "@[<2>%a@ =@ %a@]"
        Oprint.(print out_type) t
        Oprint.(print out_type) t'
...
let type_path_expansion ppf = function
  | Same p -> Oprint.(print out_ident) ppf p
  | Diff(p,p') ->
      fprintf ppf "@[<2>%a@ =@ %a@]"
         Oprint.(print out_ident) p
         Oprint.(print out_ident) p'
...
let explanation (type variety) intro prev env
  ...
  | Errortrace.Escape {kind; context} ->
    let pre =
      ...
      | Some ctx, _, _ ->
        reserve_names ctx;
        dprintf "@[%a@;<1 2>%a@]" pp_doc intro type_expr_with_reserved_names ctx
