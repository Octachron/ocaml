...
let include_err ppf =
  ...
  function
  | CM_Virtual_class ->
      fprintf ppf (f_"A class cannot be changed from virtual to concrete")
  | CM_Parameter_arity_mismatch _ ->
      fprintf ppf
        (f_"The classes do not have the same number of type parameters")
  | CM_Type_parameter_mismatch (env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (dprintf @@ f_"A type parameter has type")
        (dprintf @@ f_"but is expected to have type")
  | CM_Class_type_mismatch (env, cty1, cty2) ->
      Printtyp.wrap_printing_env env (fun () ->
        fprintf ppf
          (f_"@[The class type@;<1 2>%a@ %a@;<1 2>%a@]")
          Printtyp.class_type cty1
          i18n (s_"is not matched by the class type")
          Printtyp.class_type cty2)
  | CM_Parameter_mismatch (env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (dprintf @@ f_"A parameter has type")
        (dprintf @@ f_"but is expected to have type")
  | CM_Val_type_mismatch (lab, env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (dprintf (f_"The instance variable %s@ has type") <$> lab)
        (dprintf @@ f_"but is expected to have type")
  | CM_Meth_type_mismatch (lab, env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (dprintf (f_"The method %s@ has type") <$> lab)
        (dprintf @@ f_"but is expected to have type")
  | CM_Non_mutable_value lab ->
      fprintf ppf
       (f_"@[The non-mutable instance variable %s cannot become mutable@]") lab
  | CM_Non_concrete_value lab ->
      fprintf ppf
       (f_"@[The virtual instance variable %s cannot become concrete@]") lab
  | CM_Missing_value lab ->
      fprintf ppf (f_"@[The first class type has no instance variable %s@]") lab
  | CM_Missing_method lab ->
      fprintf ppf (f_"@[The first class type has no method %s@]") lab
  | CM_Hide_public lab ->
     fprintf ppf (f_"@[The public method %s cannot be hidden@]") lab
  | CM_Hide_virtual (k, lab) ->
      fprintf ppf (f_"@[The virtual %s %s cannot be hidden@]") k lab
  | CM_Public_method lab ->
      fprintf ppf (f_"@[The public method %s cannot become private") lab
  | CM_Virtual_method lab ->
      fprintf ppf (f_"@[The virtual method %s cannot become concrete") lab
  | CM_Private_method lab ->
      fprintf ppf (f_"The private method %s cannot become public") lab
