...
let include_err ppf =
  ...
  | CM_Type_parameter_mismatch (env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (fun ppf -> I18n.fprintf ppf "A type parameter has type")
  ...
  | CM_Parameter_mismatch (env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (fun ppf -> I18n.fprintf ppf "A parameter has type")
        (fun ppf -> I18n.fprintf ppf "but is expected to have type")
  | CM_Val_type_mismatch (lab, env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (fun ppf -> I18n.fprintf ppf "The instance variable %s@ has type" lab)
        (fun ppf -> I18n.fprintf ppf "but is expected to have type")
  | CM_Meth_type_mismatch (lab, env, trace) ->
      Printtyp.report_unification_error ppf env ~unif:false trace
        (fun ppf -> I18n.fprintf ppf "The method %s@ has type" lab)
        (fun ppf -> I18n.fprintf ppf "but is expected to have type")
  ...
  | CM_Missing_value lab ->
      I18n.fprintf ppf "@[The first class type has no instance variable %s@]" lab
