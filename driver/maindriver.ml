...
let process continue argv log ppf =
  ...
  if !make_archive then begin
    ...
    Compmisc.with_dump_log ~file_prefix:extracted_output log (fun log ->
        Bytepackager.package_files ~log (Compmisc.initial_env ())
  ...
  let exit_number =
    ...
    | _ ->
        ...
        if print_profile then
          Compmisc.with_dump_log ~file_prefix:"profile" !log
