...
and label_exp ctxt f (l,p) =
  match l with
  | Nolabel ->
      (* single case pattern parens needed here *)
      pp f "%a@ " (simple_pattern ctxt) p
  | Optional (rest,opt) | Typed_optional (rest, (_ ,opt)) ->
      let tyopt = match l with Typed_optional(_,(ty,_)) -> Some ty | _ -> None in
    ...
    if x.pexp_attributes <> [] then pp f "=@;%a" (expression ctxt) x
      ...
      | Pexp_fun (label, p, e) ->
          if label=Nolabel then
            ...
            pp f "%a@ %a"
              (label_exp ctxt) (label,p) pp_print_pexp_function e
      | Pexp_newtype (str,e) ->
  ...
  | Pstr_class l ->
      let extract_class_args cl =
        let rec loop acc cl =
          ...
          | Pcl_fun (l, p, cl') when cl.pcl_attributes = [] ->
              loop ((l,p) :: acc) cl'
          | _ -> List.rev acc, cl
        in
