...
3 |   let f r = match r with {x; y} -> y + y
                             ^^^^^^
Warning 41 [ambiguous-name]: these field labels belong to several types: "M1.u"
  "M1.t". The first one was selected. Please disambiguate if this is wrong.@
...
Warning 9 [missing-record-field-pattern]: the following labels are not bound in this record pattern:
  "y" . Either bind these labels explicitly or add "; _" to the pattern.
...
7 |   let y = {x = ""}
              ^^^^^^^^
Warning 41 [ambiguous-name]: these field labels belong to several types: "M.s"
  "t". The first one was selected. Please disambiguate if this is wrong.@
