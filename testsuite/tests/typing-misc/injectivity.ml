...
1 | type _ t = N : 'a -> 'a N.t t (* KO *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor N : 'a -> 'a N.t t the type variable 'a
...
3 | type 'a u = M : 'a -> 'a M.t u
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor M : 'a -> 'a M.t u the type variable 'a
       cannot be deduced from the type parameters.
|}]
module M : sig type !'a t = private < m : int ; .. > end =
...
3 |   type _ x = G : 'a -> 'a u x
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor G : 'a X.t -> 'a X.t u x the type variable 'a
       cannot be deduced from the type parameters.
|}]
