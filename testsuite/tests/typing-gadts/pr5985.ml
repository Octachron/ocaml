...
3 |   type _ t = T : 'a -> 'a s t
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor T : 'a -> 'a s t the type variable 'a
...
Error: In the definition type 'a c = < x : 'b > constraint 'a = 'b T.t
...
1 | type 'x t = A of 'a constraint 'x = [< `X of 'a ] ;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the definition type 'b t = A of 'a constraint 'b = [< `X of 'a ]
...
1 | type 'a t = A : 'a -> [< `X of 'a ] t;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor A : 'a -> [< `X of 'a ] t the type variable 'a
       cannot be deduced from the type parameters.
|}];;
...
4 | type _ t = T : 'a -> ('a, 'b) Ephemeron.K1.t t;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor T : 'a -> ('a, 'b) Ephemeron.K1.t t
       the type variable 'a cannot be deduced from the type parameters.
|}];;
(*
...
2 | type +'a t = 'b constraint 'a = 'b q;;
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the definition type +'a t = 'b constraint 'a = 'b q
...
1 | type -'a s = 'b constraint 'a = 'b t;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the definition type -'a s = 'b constraint 'a = 'b t
       the type variable 'b has a variance that
...
1 | type +'a s = 'b constraint 'a = 'b t q;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the definition type +'a s = 'b constraint 'a = 'b t q
       the type variable 'b has a variance that
       cannot be deduced from the type parameters.
       It was expected to be unrestricted, but it is covariant.
|}];;
...
2 | type _ g = G : 'a -> 'a t g;; (* fail *)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the GADT constructor G : 'a list -> 'a list t g the type variable
       'a cannot be deduced from the type parameters.
|}];;
