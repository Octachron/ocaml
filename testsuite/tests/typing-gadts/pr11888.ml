...
6 |   | Ns : 'a nat -> 'a s nat
Error: In the GADT constructor Nss : 'd nat -> 'd s s nat the type variable
       'd cannot be deduced from the type parameters.
|}];;
...
5 | type _ nat += Nss : 'd nat -> 'd s s nat
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: In the extension constructor type _ nat += Nss : 'd nat -> 'd s s nat
