...
let rec load_file recursive (log,_ as l) name =
  let filename =
  ...
  | Some filename ->
      ...
      Misc.try_finally
        ...
        (fun () -> really_load_file recursive l name filename ic)

and really_load_file recursive (log, _ as l) name filename ic =
          ...
          (fun (reloc, _) -> match reloc with
            | Reloc_getcompunit cu
              when not (Symtable.is_global_defined
                ...
                | exception Not_found -> ()
                | file ->
                    if not (load_file recursive l file) then raise Load_failed
                end
            | Reloc_getcompunit _
