...
let rec build_object_init ~scopes cl_table obj params inh_init obj_init cl =
  ...
  | Tcl_apply (cl, oexprs) ->
      ...
      in
      (inh_init, transl_apply ~scopes obj_init (transl_args ~scopes oexprs) Loc_unknown)
...
let rec transl_class_rebind ~scopes obj_init cl vf =
  ...
  | Tcl_apply (cl, oexprs) ->
      let path, path_lam, obj_init =
        transl_class_rebind ~scopes obj_init cl vf in
      (path, path_lam, transl_apply ~scopes obj_init (transl_args ~scopes oexprs) Loc_unknown)
  | Tcl_let (rec_flag, defs, _vals, cl) ->
      let path, path_lam, obj_init =
        transl_class_rebind ~scopes obj_init cl vf in
